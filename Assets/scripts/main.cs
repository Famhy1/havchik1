...
public class main : MonoBehaviour {
	...
	public bool[,] crit=new bool[40,40];
	...
	public void endcreatingteam(){
		creatingteam = false;
		string s1 = crtext.transform.GetChild (0).gameObject.GetComponent<InputField> ().text;
		int a = -1;
		for (int i=0; i<10; i++) {
			if(teams[i].mainunit==-1){
				a=i;
		...
		crtext.transform.GetChild (0).gameObject.GetComponent<InputField> ().text="";
			...
			if(citnow.d.usl1!=""){
				...
				for (int i=0;i<razdel.Count;i++){
					...
					if(razdel[i]=="unit"){
						...
						if (!citnow.unitsin.Contains(int.Parse (razdel[i+1])))
							bo=false;
						i+=1;
					} else if(razdel[i]=="itemin"){
						if (!citnow.resin.Contains(int.Parse (razdel[i+1])))
							bo=false;
						else if (citnow.resincol[citnow.resin.IndexOf(int.Parse (razdel[i+1]))]<int.Parse (razdel[i+2]))
							bo=false;
						i+=2;
					}
				}
				...
				dialnum=citnow.d.d1;
						...
						for (int i=0;i<citnow.d.act1.Length;i++){
							...
							for (int i=0;i<razdel.Count;i++){
								...
								if(razdel[i]=="add"){
									if(citnow.resin.Contains (int.Parse (razdel[i+1])) && citnow.resincol[citnow.resin.IndexOf (int.Parse (razdel[i+1]))]>=int.Parse (razdel[i+2])){
										resource[int.Parse (razdel[i+1])]+=int.Parse (razdel[i+2]);
										citnow.resincol[citnow.resin.IndexOf (int.Parse (razdel[i+1]))]-=int.Parse (razdel[i+2]);
									}
									i+=2;
								} else 
								if(razdel[i]=="del"){
									...
									if(citnow.resin.Contains (int.Parse (razdel[i+1])))
										...
										citnow.resin.Add (int.Parse (razdel[i+1]));
										citnow.resincol.Add (int.Parse (razdel[i+2]));
									}
									i+=2;
								} else 
								if(razdel[i]=="exit"){
								...
								if(razdel[i]=="delunit"){
									...
									citnow.unitsin.Add (int.Parse (razdel[i+1]));
								...
								if(razdel[i]=="addunit"){
									if(citnow.unitsin.Contains (int.Parse (razdel[i+1]))){
										...
										teams[chosedteam].inst.Add (h);
										teams[chosedteam].now+=0.1f;
										citnow.unitsin.RemoveAt(citnow.unitsin.IndexOf (int.Parse (razdel[i+1])));
									}
									i+=1;
								}else 
								if(razdel[i]=="downdefence"){
									if(citnow.uns.Count>0){
										Destroy (citnow.uns[citnow.uns.Count-1]);
										citnow.uns.RemoveAt (citnow.uns.Count-1); Destroy (citnow.unsotkat[citnow.uns.Count-1]); citnow.unsotkat.RemoveAt (citnow.uns.Count-1);
									}
								} else 
								if(razdel[i]=="sdav"){
									if(citnow.race!=11){
								...
								if(razdel[i]=="destroy"){
									...
									for(int j1=0;j1<citnow.resin.Count;j1++){
										int yur=rnd.Next (2);
										if(yur==1){
											int yur1=rnd.Next (citnow.resincol[j1]);
											resource[citnow.resin[j1]]+=yur1;
											citnow.resincol[j1]-=yur1;
										}
									}
